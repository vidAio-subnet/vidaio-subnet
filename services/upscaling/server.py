...
async def video_upscaler(request: UpscaleRequest):
    ...
    try:
        ...
        # Step 1: Duplicate the last two frames in the .mp4 file
        print("Step 1: Duplicating last two frames in the .mp4 file...")
        start_time = time.time()
        duplicate_frames_command = [
            "ffmpeg",
            "-i", str(input_file),
            "-vf", "tpad=stop_mode=clone:stop_duration=2",
            str(output_file_with_extra_frames)
        ]
        duplicate_frames_process = subprocess.run(duplicate_frames_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        elapsed_time = time.time() - start_time
        if duplicate_frames_process.returncode != 0:
            print(f"Duplicating frames failed: {duplicate_frames_process.stderr.strip()}")
            raise HTTPException(status_code=500, detail=f"Duplicating frames failed: {duplicate_frames_process.stderr.strip()}")
