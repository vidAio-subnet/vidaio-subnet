...
# --- vmaf_data_augmentation imports MOVED into create_preprocessing_pipeline_with_augmentation ---
# from vmaf_data_augmentation import smart_augmentation,targeted_augmentation,quality_aware_augmentation,cross_content_augmentation

# Make sure we can import from parent directory if needed
# sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))) # Redundant
...
# 2. Improved Resolution Transformer with better format handling
...
# 3. Improved Feature Scaler with better handling of missing values and edge cases
...
# 4. Improved Frame Rate Transformer
...
# 5. Complete preprocessing pipeline with improved components

def create_preprocessing_pipeline_with_augmentation(input_file=None, output_file=None, pipeline_file=None,
                                                  target_column='vmaf', codec_type='auto',
                                                  augment_data=True,augmentation_type='smart', augmentation_factor=1.5,
                                                  visualize_augmentation=False,verbose=True):
    """
    Create and fit an improved preprocessing pipeline with better CQ and resolution handling,
    and optional data augmentation via CQ-VMAF interpolation.

    Args:
        input_file: Path to input CSV file
        output_file: Path to output preprocessed CSV file
        pipeline_file: Path to save the fitted pipeline
        target_column: Name of the target column (default: 'vmaf')
        codec_type: Type of codec ('auto', 'av1', 'x264', etc.) for CQ scaling
        augment_data: Whether to apply data augmentation (default: True)
        augmentation_factor: Target factor by which to increase dataset size (default: 1.5)
        visualize_augmentation: Whether to visualize augmentation results (default: False)
        verbose: Whether to print detailed logs (default: True)

    Returns:
        tuple: (pipeline, X_train, y_train)
            - pipeline: The fitted or unfitted scikit-learn Pipeline object.
            - X_train: DataFrame of processed features (None if no input_file).
            - y_train: Series of processed target variable (None if no input_file).
    """
    import pandas as pd # Keep pandas import here if only used here, or move to top if used elsewhere
    import os # Keep os import here if only used here, or move to top if used elsewhere
    # from sklearn.pipeline import Pipeline # Already imported at top
    # from sklearn.utils.validation import NotFittedError,check_is_fitted # Already imported at top
    ...
    if augment_data:
        try:
            ...
            if verbose:
                print("ERROR: 'vmaf_data_augmentation' module not found. Augmentation will be skipped.")
            augment_data = False # Disable augmentation if module is missing
            visualize_augmentation = False
        
        if visualize_augmentation and augment_data: # Also check if augment_data is still true
            try:
    ...
    # If input file is provided, load and potentially augment the data
    if input_file:
        if verbose: print(f"Reading data from {input_file}...") # Wrap print
        try:
            df = pd.read_csv(input_file)
        except FileNotFoundError:
            if verbose: print(f"ERROR: Input file not found at {input_file}")
            return None, None, None
        except Exception as e:
            if verbose: print(f"ERROR: Failed to read input file {input_file}: {e}")
            return None, None, None


        if verbose: # Wrap print block
            print(f"Loaded {df.shape[0]} rows with {df.shape[1]} columns")
            if 'cq' in df.columns:
                print(f"CQ range: {df['cq'].min()} to {df['cq'].max()}")
            if target_column in df.columns:
                print(f"VMAF range: {df[target_column].min()} to {df[target_column].max()}")


        if df is not None and augment_data and 'cq' in df.columns and target_column in df.columns and 'original_segment_path' in df.columns:
            if verbose: print("\n--- Performing Data Augmentation ---") # Wrap print

            # Auto-detect codec type if set to 'auto'
            detected_codec = codec_type
            if codec_type == 'auto':
                cq_min, cq_max = df['cq'].min(), df['cq'].max()
                temp_scaler = CQScaler() 
                detected_codec = temp_scaler._detect_codec(cq_min, cq_max)
                del temp_scaler 

                if verbose: print(f"Using codec type '{detected_codec}' for augmentation") 

            original_df_for_viz = df.copy() if visualize_augmentation else None 

            if augmentation_type=='smart':
                df = smart_augmentation(
                    df,
                    augmentation_factor=augmentation_factor,
                    codec_type=detected_codec,
                    visualize=visualize_augmentation, 
                )
            elif augmentation_type=='targeted':
                df = targeted_augmentation(df) 

            elif augmentation_type=='quality-aware':
                df = quality_aware_augmentation(df) 

            else: 
                df = cross_content_augmentation(df) 

            if visualize_augmentation and original_df_for_viz is not None:
                # Define plot_cq_vmaf_comparison here or ensure it's imported conditionally
                def plot_cq_vmaf_comparison(original_df_plot, augmented_df_plot, n_samples_plot=5):
                    """
                    Plot a comparison of CQ-VMAF curves before and after augmentation
                    """
                    # import matplotlib.pyplot as plt # Already imported above if visualize_augmentation is True
                    # import numpy as np # Already imported at top
                    
                    segments = original_df_plot['original_segment_path'].unique()
                    if len(segments) > n_samples_plot:
                        np.random.seed(42) 
                        sample_segments = np.random.choice(segments, n_samples_plot, replace=False)
                    else:
                        sample_segments = segments
                    
                    fig, axes = plt.subplots(1, len(sample_segments), figsize=(5*len(sample_segments), 5))
                    ...
                    for i, segment in enumerate(sample_segments):
                        ...
                        orig_segment_df = original_df_plot[original_df_plot['original_segment_path'] == segment].sort_values(by='cq')
                        aug_segment_df = augmented_df_plot[augmented_df_plot['original_segment_path'] == segment].sort_values(by='cq')
                        
                        # Plot original data
                        ax.scatter(orig_segment_df['cq'], orig_segment_df['vmaf'], 
                                  color='blue', marker='o', label='Original')
                        ax.plot(orig_segment_df['cq'], orig_segment_df['vmaf'], 
                               'b--', alpha=0.5)
                        
                        # Plot augmented data
                        ax.scatter(aug_segment_df['cq'], aug_segment_df['vmaf'], 
                                  color='red', marker='x', label='Augmented')
                        ax.plot(aug_segment_df['cq'], aug_segment_df['vmaf'], 
                               'r-', alpha=0.3)
                        
                        # Add labels and formatting
                        ax.set_xlabel('CQ Value')
                        ax.set_ylabel('VMAF Score')
                        ax.set_title(f'Segment {os.path.basename(segment)[:20]}...') # Shorter title
                        ax.grid(True, alpha=0.3)
                        ax.legend()
                        
                        # Invert x-axis so lower CQ (higher quality) is on the right
                        ax.invert_xaxis()
                    
                    plt.suptitle('CQ-VMAF Curves: Original vs Augmented Data', fontsize=16)
                    plt.tight_layout()
                    plt.subplots_adjust(top=0.85)
                    plt.show()

                plot_cq_vmaf_comparison(original_df_for_viz, df) 

        for col in ['is_augmented', 'is_extrapolated', 'is_targeted']: 
            if col in df.columns:
                df = df.drop(col, axis=1)

    # Create the specialized scalers
    vmaf_scaler = VMAFScaler(target_column, verbose=verbose)
    cq_scaler = CQScaler('cq', codec=codec_type, verbose=verbose)

    # Define the preprocessing steps, passing verbose
    pipeline = Pipeline([
        ('column_dropper', ColumnDropper(columns_to_drop)),
        ('resolution_transformer', ResolutionTransformer('metrics_resolution', verbose=verbose)),
        ('frame_rate_transformer', FrameRateTransformer('metrics_frame_rate', verbose=verbose)),
        ('cq_scaler', cq_scaler),
        ('vmaf_scaler', vmaf_scaler),
        ('feature_scaler', FeatureScaler(feature_columns,
                                         scaling_type='minmax',
                                         excluded_columns=[target_column, 'cq'],
                                         verbose=verbose)),
        ('target_extractor', TargetExtractor(target_column, vmaf_scaler)), # Pass vmaf_scaler here if needed by TargetExtractor
    ])

    # If we have data, fit the pipeline and transform
    if df is not None:
        # Fit the pipeline
        if verbose: print("\n--- Fitting Preprocessing Pipeline ---") # Wrap print
        try:
            pipeline.fit(df)
            # --- MODIFY LOGGING CHECK ---
            if verbose:
                is_fitted_status = "Unknown"
                try:
                    # Check the INTERNAL scaler of the FeatureScaler step
                    check_is_fitted(pipeline.named_steps['feature_scaler'].scaler)
                    is_fitted_status = "Yes (internal scaler checked)"
                except NotFittedError:
                    is_fitted_status = "No (check_is_fitted failed on internal scaler)"
                except (AttributeError, KeyError):
                     is_fitted_status = "Unknown (internal scaler or step missing)"

                print(f"Pipeline fitting completed. Is fitted? {is_fitted_status}")
            # --- END MODIFIED LOGGING CHECK ---
        except Exception as e:
             if verbose: print(f"ERROR during pipeline fitting: {e}")
             # Decide how to handle fitting error - maybe return None or raise?
             return None, None, None # Example: return None on fitting error

        # Transform the data
        try:
            transformed_df = pipeline.transform(df)
        except Exception as e:
            if verbose: print(f"ERROR during pipeline transform after fitting: {e}")
            return None, None, None # Return None if transform fails

        # Verify scaling has worked properly
        if verbose: # Wrap print block
            print("\nVerifying scaling:")
            scaling_columns = ['cq', target_column, 'metrics_resolution', 'metrics_frame_rate']
            scaling_columns.extend([col for col in feature_columns if col in transformed_df.columns]) # Add feature columns
            for col in scaling_columns:
                if col in transformed_df.columns:
                    ...
                    if pd.isna(col_min) or pd.isna(col_max):
                         print(f"  Warning: {col} contains NaN values after scaling!")
                    elif col_min < -1e-6 or col_max > 1 + 1e-6: # Allow for small float inaccuracies
                        print(f"  Warning: {col} scaling may not be working correctly (outside [0, 1])!")
                else:
                    print(f"  Warning: Column '{col}' not found in transformed_df for verification.")
        ...
        if target_column in transformed_df.columns:
            X_train = transformed_df.drop(columns=[target_column])
            if verbose: print(f"Removed target column '{target_column}' from feature set X_train.")
        else:
             X_train = transformed_df # Should not happen if target_column exists initially
             if verbose: print(f"Warning: Target column '{target_column}' not found in transformed_df.")
        ...
        if output_file:
            ...
            try:
                ...
                # Decide what to save: Just features (X_train) or features + target (y_train)?
                # Saving features + target is often useful for inspection.
                output_df_to_save = X_train.copy()
                if y_train is not None:
                     output_df_to_save[target_column] = y_train # Add the extracted target back
                output_df_to_save.to_csv(output_file, index=False)
            except Exception as e:
                 if verbose: print(f"ERROR: Failed to save preprocessed data to {output_file}: {e}")


        # If pipeline file is provided, save the fitted pipeline
        if pipeline_file:
            if verbose: print(f"\nSaving preprocessing pipeline to {pipeline_file}") # Wrap print
            try:
                os.makedirs(os.path.dirname(pipeline_file), exist_ok=True)
                with open(pipeline_file, 'wb') as f:
                    pickle.dump(pipeline, f)
            except Exception as e:
                 if verbose: print(f"ERROR: Failed to save pipeline to {pipeline_file}: {e}")


    else:
        # --- ADD LOGGING ---
        if verbose:
            print("WARNING: No input data (df is None), pipeline was NOT fitted.")
        # --- END LOGGING ---

    
    # --- MODIFY LOGGING BEFORE RETURN ---
    if verbose:
        is_fitted_status_final = "Unknown"
        try:
            # Check the INTERNAL scaler of the FeatureScaler step
            check_is_fitted(pipeline.named_steps['feature_scaler'].scaler)
            is_fitted_status_final = "Yes (internal scaler checked)"
        except (NotFittedError, AttributeError, KeyError):
             is_fitted_status_final = "No"
        except NameError:
             is_fitted_status_final = "Unknown (check_is_fitted not available)"

        print(f"Returning pipeline. Is fitted? {is_fitted_status_final}")
